...
SscmaCamera::SscmaCamera(esp_io_expander_handle_t io_exp_handle)
{
    ...
    callback.on_connect = [](sscma_client_handle_t client, const sscma_client_reply_t *reply, void *user_ctx) { ESP_LOGI(TAG, "SSCMA client connected"); };

    callback.on_log = [](sscma_client_handle_t client, const sscma_client_reply_t *reply, void *user_ctx) { ESP_LOGI(TAG, "log: %s\n", reply->data); };
    ...
    {
        ESP_LOGI(TAG, "Device Info - ID: %s, Name: %s", info->id ? info->id : "NULL", info->name ? info->name : "NULL");
    }
    // 初始化JPEG数据的内存
    jpeg_data_.len = 0;
...
CameraJpegData SscmaCamera::GetJpegData()
{
    CameraJpegData result = {nullptr, 0};

    if (jpeg_data_.buf != nullptr && jpeg_data_.len > 0)
    {
        result.data = jpeg_data_.buf;
        result.size = jpeg_data_.len;
    }
    else
    {
        ESP_LOGE(TAG, "No JPEG data available");
    }

    return result;
}

CameraRawData SscmaCamera::GetRawData()
{
    CameraRawData result = {nullptr, 0, 0, 0, 0};

    // SscmaCamera 已经提供JPEG数据，所以我们返回JPEG数据作为"原始"数据
    if (jpeg_data_.buf != nullptr && jpeg_data_.len > 0)
    {
        result.data = jpeg_data_.buf;
        result.size = jpeg_data_.len;
        result.width = 240;             // 假设的宽度，可以根据实际情况调整
        result.height = 240;            // 假设的高度，可以根据实际情况调整
        result.format = PIXFORMAT_JPEG; // SscmaCamera提供的是JPEG格式
        ESP_LOGI(TAG, "Returning SSCMA raw data (JPEG): %dx%d, size: %d bytes", result.width, result.height, result.size);
    }
    else
    {
        ESP_LOGE(TAG, "No raw data available from SSCMA camera");
    }
