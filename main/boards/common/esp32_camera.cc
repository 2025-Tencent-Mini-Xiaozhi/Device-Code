...
CameraJpegData Esp32Camera::GetJpegData()
{
    CameraJpegData result = {nullptr, 0};
    ...
    if (fb_->format == PIXFORMAT_JPEG)
    {
        ...
        result.data = fb_->buf;
        result.size = fb_->len;
        ESP_LOGI(TAG, "Returning JPEG data directly, size: %d bytes", fb_->len);
    }
    else
    {
        ESP_LOGI(TAG, "Frame buffer format: %d, converting to JPEG", fb_->format);
        // 转换为JPEG格式
        uint8_t *jpeg_buf = nullptr;
        size_t jpeg_len = 0;

        if (frame2jpg(fb_, 80, &jpeg_buf, &jpeg_len))
        {
            // 注意：这里返回的是动态分配的内存，调用者需要负责释放
            // 但是由于CameraJpegData结构体使用const uint8_t*，我们需要另一种方法
            // 让我们使用一个静态缓冲区来存储转换后的JPEG数据
            static uint8_t *static_jpeg_buf = nullptr;
            static size_t static_jpeg_buf_size = 0;

            // 释放之前的缓冲区
            if (static_jpeg_buf != nullptr)
            {
                free(static_jpeg_buf);
            }

            // 保存新的缓冲区
            static_jpeg_buf = jpeg_buf;
            static_jpeg_buf_size = jpeg_len;

            result.data = static_jpeg_buf;
            result.size = static_jpeg_buf_size;
            ESP_LOGI(TAG, "Converted to JPEG, size: %d bytes", jpeg_len);
        }
        else
        {
...
CameraRawData Esp32Camera::GetRawData()
{
    CameraRawData result = {nullptr, 0, 0, 0, 0};
    ...
    result.data = fb_->buf;
    result.size = fb_->len;
    result.width = fb_->width;
    result.height = fb_->height;
    result.format = fb_->format;

    ESP_LOGI(TAG, "Returning raw data: %dx%d, format: %d, size: %d bytes", fb_->width, fb_->height, fb_->format, fb_->len);
