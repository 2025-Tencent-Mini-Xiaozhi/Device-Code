...
WifiConfigurationAp::WifiConfigurationAp()
{
    ...
    if (err == ESP_OK)
    {
        ...
        err = nvs_get_str(nvs, "ota_url", nullptr, &required_size);
        if (err == ESP_OK && required_size > 1)
        {
            char *ota_url_buf = (char *)malloc(required_size);
            if (ota_url_buf)
            {
                err = nvs_get_str(nvs, "ota_url", ota_url_buf, &required_size);
                if (err == ESP_OK)
                {
                    ota_url_ = std::string(ota_url_buf);
                    ESP_LOGI(TAG, "Loaded OTA URL from NVS: %s", ota_url_.c_str());
                }
                free(ota_url_buf);
            }
        }
        ...
        // 加载WiFi功率设置
        int8_t saved_power;
        err = nvs_get_i8(nvs, "max_tx_power", &saved_power);
        if (err == ESP_OK)
        {
            max_tx_power_ = saved_power;
            ESP_LOGI(TAG, "Loaded WiFi power from NVS: %d", max_tx_power_);
        }

        // 加载BSSID记忆设置
        uint8_t saved_bssid;
        err = nvs_get_u8(nvs, "remember_bssid", &saved_bssid);
        if (err == ESP_OK)
        {
            remember_bssid_ = (saved_bssid != 0);
            ESP_LOGI(TAG, "Loaded remember_bssid from NVS: %s", remember_bssid_ ? "true" : "false");
        }
    ...
    {
        ESP_LOGI(TAG, "No saved advanced config found, using defaults");
    }
}
...
void WifiConfigurationAp::Start()
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &WifiConfigurationAp::WifiEventHandler, this, &instance_any_id_));
    ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &WifiConfigurationAp::IpEventHandler, this, &instance_got_ip_));
...
std::string WifiConfigurationAp::GetOtaUrl() const { return ota_url_; }
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t scan = {.uri = "/scan",
                        ...
                        .handler = [](httpd_req_t *req) -> esp_err_t
                        {
                            ...
                            {
                                ESP_LOGI(TAG, "SSID: %s, RSSI: %d, Authmode: %d", (char *)this_->ap_records_[i].ssid, this_->ap_records_[i].rssi, this_->ap_records_[i].authmode);
                                char buf[128];
                                snprintf(buf, sizeof(buf), "{\"ssid\":\"%s\",\"rssi\":%d,\"authmode\":%d}", (char *)this_->ap_records_[i].ssid, this_->ap_records_[i].rssi, this_->ap_records_[i].authmode);
    ...
    httpd_uri_t form_submit = {.uri = "/submit",
                               .method = HTTP_POST,
                               .handler = [](httpd_req_t *req) -> esp_err_t
                               {
                                   char *buf;
    ...
    httpd_uri_t advanced_submit = {.uri = "/advanced/submit",
                                   .method = HTTP_POST,
                                   .handler = [](httpd_req_t *req) -> esp_err_t
                                   {
                                       char *buf;
...
{
    // 注册SmartConfig事件处理器
    ESP_ERROR_CHECK(esp_event_handler_instance_register(SC_EVENT, ESP_EVENT_ANY_ID, &WifiConfigurationAp::SmartConfigEventHandler, this, &sc_event_instance_));
